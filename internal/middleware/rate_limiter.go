...
func (rl *RateLimiter) GetRemaining(ctx context.Context, endpoint, key string) (int, error) {
	config, ok := rl.configs[endpoint]
	if !ok {
		return 0, nil
	}
	...
	// Удаляем старые записи и считаем текущие
	pipe := rl.client.Pipeline()
	pipe.ZRemRangeByScore(ctx, windowKey, "0", strconv.FormatInt(windowStart.UnixNano(), 10))
	countCmd := pipe.ZCard(ctx, windowKey)
...
func (rl *RateLimiter) Reset(ctx context.Context, endpoint, key string) error {
	config, ok := rl.configs[endpoint]
	if !ok {
		return nil
	}
	windowKey := config.Prefix + key
	return rl.client.Del(ctx, windowKey).Err()
}
